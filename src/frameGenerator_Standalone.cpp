...
#include "../resource/materials/texturedLit_OPENGL.inc"
};
...
const int FRAME_TO_SKIP = 1;
static int g_materialVariantCount = 10;
static std::string g_prefix = "screenshot";
...
SDL_Window* createSDLwindow() {
    ...
    const uint32_t windowFlags =
            SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI | SDL_WINDOW_HIDDEN;
    SDL_Window* win =
    ...
    if (frame >= 1 && frame < g_materialVariantCount + 1) {
        ...
        backend::PixelBufferDescriptor buffer(pixels, vp.width * vp.height * 3,
                ...
                [](void* buffer, size_t size, void* user) {
                    ...
                    image::LinearImage image(image::toLinear<uint8_t>(v.width, v.height, v.width * 3,
                    ...
                    image::ImageEncoder::encode(outputStream, image::ImageEncoder::Format::PNG, image, "", name);
...
int main() {
    ...
    SwapChain* swapChain = engine->createSwapChain(1024, 640);
